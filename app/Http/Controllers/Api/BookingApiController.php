...
class BookingApiController extends Controller
{
    ...
    public function updateAppointment(Request $request)
    {
        try {
            ...
            // Handle date update
            if (isset($validated['new_date']) && !empty($validated['new_date'])) {
                $time = isset($validated['new_time']) && !empty($validated['new_time']) 
                    ? $validated['new_time'] 
                    : $booking->appointment_time->format('H:i');
                
                $newDateTime = $validated['new_date'] . ' ' . $time;
                ...
                if (!$this->bookingService->isSlotAvailableForBooking($newDateTime, $booking->id)) {
                    return response()->json([
                        'success' => false,
                        'message' => 'The requested time slot is not available.',
                        'alternatives' => $this->bookingService->getAlternativeSlots($validated['new_date']),
                ...
                $updateData['appointment_time'] = $newDateTime;
                $updatedFields[] = 'date';
            }

            // Handle time update only
            if (isset($validated['new_time']) && !empty($validated['new_time']) && !isset($validated['new_date'])) {
                $date = $booking->appointment_time->format('Y-m-d');
                $newDateTime = $date . ' ' . $validated['new_time'];
                ...
                if (!$validationResult['valid']) {
                    return response()->json([
                        'success' => false,
                        'message' => $validationResult['message'],
                        'alternatives' => $this->bookingService->getAlternativeSlots($date),
                    ], 400);
                }
            ...
            if (isset($validated['new_service']) && !empty($validated['new_service'])) {
                $updateData['service'] = $validated['new_service'];
                $updatedFields[] = 'service';
            }

            if (isset($validated['new_name']) && !empty($validated['new_name'])) {
                $updateData['name'] = $validated['new_name'];
                $updatedFields[] = 'name';
            }

            if (isset($validated['new_phone']) && !empty($validated['new_phone'])) {
                $updateData['phone'] = $validated['new_phone'];
                $updatedFields[] = 'phone';
            }

            if (isset($validated['new_email']) && !empty($validated['new_email'])) {
                $updateData['email'] = $validated['new_email'];
                $updatedFields[] = 'email';
            }

            if (isset($validated['new_notes']) && !empty($validated['new_notes'])) {
            ...
            if (empty($updateData)) {
                return response()->json([
                    'success' => false,
                    'message' => 'No valid fields to update. Please provide at least one field.',
                ], 422);
            }
            ...
            if ($booking->google_event_id) {
                try {
                    $this->bookingService->updateGoogleCalendarEvent($booking, $updateData);
                    $googleUpdated = true;
                } catch (\Exception $e) {
            ...
            foreach ($updatedFields as $field) {
                if (isset($fieldNames[$field])) {
                    $updatedFieldNames[] = $fieldNames[$field];
                }
            }
            
            $message = 'Appointment updated successfully!';
            if (!empty($updatedFieldNames)) {
                $message = 'Appointment ' . implode(', ', $updatedFieldNames) . ' updated successfully!';
            }
